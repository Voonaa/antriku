...
export default function AdminQueues({ antrians, stats }) {
    const { t } = useLang();
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Antrian - Admin" />
            <div className="mb-8 flex items-center justify-between flex-wrap gap-4">
                <div>
                    <h1 className="text-2xl font-black text-slate-800">{t('queues')}</h1>
                    <p className="text-sm text-slate-500">Semua antrian hari ini di instansi Anda.</p>
                </div>
                <a href={`/admin/laporan?tanggal=${tanggal}`} className="bg-primary hover:bg-teal-700 text-white font-bold py-2 px-5 rounded-xl text-sm flex items-center gap-2 transition-colors">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                <div className="p-5 border-b border-slate-100 flex flex-wrap gap-3 items-center justify-between">
                    <input type="text" placeholder="Cari nomor atau layanan..." value={search} onChange={e=>setSearch(e.target.value)} className="border border-slate-200 rounded-xl px-4 py-2 text-sm w-64 focus:border-primary outline-none"/>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-slate-600">
                        <thead className="bg-slate-50 text-xs uppercase font-bold text-slate-500 border-b border-slate-100">
                            <tr><th className="px-6 py-4">Nomor</th><th className="px-6 py-4">Layanan</th><th className="px-6 py-4">Loket</th><th className="px-6 py-4">Status</th><th className="px-6 py-4">Ambil Tiket</th><th className="px-6 py-4">Dipanggil</th></tr>
                        </thead>
                            ...
                            {filtered.map(a=>{
                                ...
                                return(
                                    <tr key={a.id} className="hover:bg-slate-50/50">
                                        ...
                                        <td className="px-6 py-4 font-mono text-slate-500">{a.waktu_panggil}</td>
                                    </tr>
                                );
                            })}
